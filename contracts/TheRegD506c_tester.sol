...
contract TheRegD506c_tester is RegD506c, Ownable {
    ...
    returns (bool) 
  {
    ...
    if (issuanceDate[_token] != 0) 
      require(now <= issuanceDate[_token] + 10 seconds);

    // Enforce shareholder limits
    if (RegD506cToken(_token).isFund())
      require(RegD506cToken(_token).shareholderCountAfter(_from, _to, _value) <= 1);
    else
      require(RegD506cToken(_token).shareholderCountAfter(_from, _to, _value) <= 3);
